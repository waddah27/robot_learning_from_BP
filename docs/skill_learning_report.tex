...
This separates \emph{what} the skill is from \emph{how fast} it runs, which is what lets the same prior be executed at
different speeds.
...
exponential rate, a cross-term between position and velocity error is added:
\begin{equation}
...
\noindent Figure~\ref{fig:stab} illustrates the nominal closed-loop behaviour for
...
\emph{material model}, not the controller. The scalpel blade and the workpiece were both
...
material). The reaction the material exerts on the blade is modeled as a saturating
...
\label{eq:cut}
\end{equation}
$d\unit{m}$ is how far the scalpel tip sits below the (live, current) material
