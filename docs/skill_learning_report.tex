...
energy-tank passivity mechanism at the joint torque port. The contribution comprises three elements. First, a \emph{variable impedance
...
Third, a formal \emph{stability and passivity} analysis by projecting the torque command onto a runtime energy-tank
...
robot passivity and safety-aware interaction with human. Exponential convergence is derived under the stated regulation assumptions and is reported
empirically as a diagnostic on the regulated task error. An important consideration for the studied approach is the data repeatability to decide
...
provides the interaction force in simulation. The experiment framework is evaluated on three materials (cork, PVC, penoplex) for both a fixed and a moving workpiece
scenarios.
\end{abstract}
...
$\ell_\phi$ & forward phase-search horizon & dimensionless \\
...
\begin{equation}
m_\Gamma
=\min_{\chi\in\{X,Y,Z\}}\Gamma_\chi
-\max_{\chi\in\{F_x,F_y,F_z\}}\Gamma_\chi
\ge0.92-0.27=0.65>0 .
...
\end{equation}
The position-correlation range $0.92$--$0.98$ therefore corresponds to an
estimated phase-locked ratio of $11.5$--$49$, whereas the force-correlation
range $0.04$--$0.27$ corresponds to only $0.04$--$0.37$. Independently, the
depth regulation score $\mathcal R_Z=0.73$ gives
$\operatorname{SNR}_Z\approx2.70$, while
$\mathcal R_{F_x},\mathcal R_{F_y},\mathcal R_{F_z}\approx0$ gives a
near-zero phase-locked force ratio. The two estimators use different variance
aggregations, but both produce the same ordering.
...
\paragraph{highlighting the contribution of the learned variance
...
normalised stiffness allocation
\begin{equation}
u(\phi)
...
scalar denominator is the full admissible stiffness range. Substitution of
\eqref{eq:precision} gives the identified allocation $u=u_w$, with
\[
...
\end{equation}
where $\Pi(\phi)$ is a $3\times3$ permutation matrix. Thus inversion reverses
the regulation ranking, whereas permutation preserves the set of gain values
but assigns them to different Cartesian axes. The direct-force condition does
not use $u=u_w$; it disables the learned-gain mode and uses the fixed force-QP
weighting of the baseline controller.

\begin{proposition}[Optimality of the matched precision structure]
...
\end{equation}
Among all measurable allocations $u:[0,1]\rightarrow[0,1]^3$, the identified
assignment $u^{\rm true}=u_w$ is the unique minimiser up to sets of zero measure.
...
The matrix $H(\phi)$ represents phase- and axis-dependent task relevance. In
the cutting experiment its depth-axis entry is dominant because depth is the
repeatable, task-critical coordinate. Proposition~\ref{prop:precision_structure}
establishes the allocation result for the stated quadratic precision objective;
whether that structural advantage produces lower closed-loop cutting error is
then tested by the paired experiment rather than assumed from the proposition.
...
\begin{equation}
\hat\phi=\arg\min_{\varphi\in[\phi,\,\phi+\ell_\phi]}
...
phase $\varphi$ assumes it \emph{should be}; $\ell_\phi$ is the forward
phase-search horizon. The first term is the position mismatch
...
T_k+\big(P_{\rm dis}-P_{\rm inj}\big)h_c,\,
T_{\min},\,T_{\max}\right),
\qquad
...
$\mathcal D_{\rm mat}=\{(d_j,y_j,\omega_j)\}_{j=1}^{N_{\rm mat}}$ contain the penetration
$d_j$, measured force magnitude $y_j=\norm{F_j}$ and non-negative sample weight
$\omega_j$. The two material parameters are obtained from the constrained nonlinear
least-squares problem
\begin{equation}
\hat\theta_{\rm mat}
=\arg\min_{\substack{k>0,\;F^{{\rm cut}}>0}}
\sum_{j=1}^{N_{\rm mat}}\omega_j
...
\begin{equation}
\hat k_{\rm mat}
=\frac{\sum_{j\in\mathcal L}\omega_jd_jy_j}
       {\sum_{j\in\mathcal L}\omega_jd_j^2},
\qquad
\hat F_{\rm mat}^{\rm cut}
=\frac{\sum_{j\in\mathcal P}\omega_jy_j}
       {\sum_{j\in\mathcal P}\omega_j}.
...
requires
$\sum_{j\in\mathcal L}\omega_jd_j^2>0$ and
$\sum_{j\in\mathcal P}\omega_j>0$.
