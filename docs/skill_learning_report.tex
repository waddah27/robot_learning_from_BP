...
\caption{Controlled comparison of gain assignments on cork ($N_{\rm seed}=11$
...
\midrule
\textbf{Identified precision} & $\mathbf{29.39 \pm 46.72}$ & --- & --- \\
Permuted precision & $31.25 \pm 47.54$ & $6.0\%$ & $0.001$ \\
Inverted precision & $44.39 \pm 47.65$ & $33.8\%$ & $0.001$ \\
Direct force-tracking & $47.84 \pm 51.73$ & $38.6\%$ & $0.002$ \\
\bottomrule
\end{tabular}
\caption{The identified precision has lower mean cut-depth error than all three
comparators. It is lower in all 11 paired seeds against the permuted and
inverted assignments ($p=0.001$), and in 10 of 11 against the direct-force
baseline ($p=0.002$). The large dispersion requires interpreting this as a
paired allocation result rather than a claim of uniformly small absolute
error.}
...
\end{equation}
which gives $6.0\%$, $33.8\%$ and $38.6\%$ for the permuted, inverted and
...
For the permuted and inverted comparator conditions, the evaluated pairs
satisfy $G_{s,c}>0$ for all $s\in\{1,\ldots,11\}$. The exact two-sided paired
...
\end{equation}
For these two comparators every observed difference is positive, so
$W^-_c=0$. Under the Wilcoxon
...
\eqref{eq:wilcoxon_exact}. The direct-force baseline is excluded from this
proposition because one of its 11 paired differences is negative; its
separately computed exact test gives $p=0.002$.
\end{proof}
...
\end{equation}
The demonstrations in this study encode a nominally straight cut. Direct use
of the GMR position mean introduced a small transverse curvature: for cork,
the 300-sample mean has a $7.066$\,mm RMS orthogonal distance from its
three-dimensional principal line (maximum $13.010$\,mm), although its path
length is only $2.25\%$ greater than its endpoint distance. Trimming symmetric
fractions from the record did not reveal a distinct approach or withdrawal
segment; the curvature persists over the central cutting samples. The force
magnitude is already $42.586$\,N at the first sample, remains non-zero
throughout, and reaches the cutting plateau toward the end. Hence there is no
force-based evidence that the stored prior contains an initial free-space
approach. The curvature is therefore not treated as part of the intended task
geometry.

The corrected learning pipeline does not truncate demonstrations by a shared
sample index. For demonstration $i$, it first computes endpoint progress
\begin{equation}
s_{i,k}=
\operatorname{clip}_{[0,1]}
\frac{(p_{i,k}-p_{i,0})^\top(p_{i,n_i-1}-p_{i,0})}
{\norm{p_{i,n_i-1}-p_{i,0}}_2^2},
\qquad
\bar s_{i,k}=\max_{\ell\leq k}s_{i,\ell},
\label{eq:progress_registration}
\end{equation}
and interpolates every pose, wrench and variability channel on a common
uniform grid in $\bar s_i$. Thus equal phase denotes equal spatial progress
rather than equal sample number.

A single structured GMM/GMR is learned from all materials. Material identity is
not represented by the ordinal scalar $1,2,3$. Instead, with cork as reference,
the categorical contrast is
\[
c(\mathrm{cork})=(0,0),\quad
c(\mathrm{penoplex})=(1,0),\quad
c(\mathrm{PVC})=(0,1),
\]
and the conditioning vector is
\begin{equation}
u(\phi,m)=
\begin{bmatrix}\phi&c_1(m)&c_2(m)&\phi c_1(m)&\phi c_2(m)\end{bmatrix}^{\!\top}.
\label{eq:material_condition}
\end{equation}
The interaction terms permit material-dependent affine endpoints and cutting
directions without imposing an artificial ordinal distance between materials.
All variables are standardized before estimation.

The shared model has an affine one-component positional block, for which
Gaussian conditioning gives an affine conditional mean, and a 15-component
wrench--orientation--twist block for nonlinear phase profiles. Both blocks are
trained jointly across the complete material data set and conditioned on the
same $u(\phi,m)$; they form one material-conditioned structured prior rather
than three material-specific models. This is a training-time GMR formulation,
not a controller-side projection. The saved conditioned position priors for all
three materials have zero orthogonal residual to numerical precision and
path-length/chord ratio one. The controller consumes them directly; no
downstream straightening operation remains.
...
& & S & T & S & T \\
\midrule
Static & off & $2.485\pm1.029$ & $2.538\pm1.075$
             & $12.250\pm10.461$ & $12.533\pm11.229$ \\
Static & on  & $3.165\pm0.795$ & $3.694\pm1.153$
             & $36.422\pm21.713$ & $48.449\pm42.406$ \\
Moving & off & $9.557\pm8.805$ & $9.631\pm8.981$
             & $21.602\pm19.391$ & $22.573\pm22.839$ \\
Moving & on  & $10.821\pm7.495$ & $12.209\pm7.085$
             & $73.324\pm15.578$ & $96.417\pm28.339$ \\
\bottomrule
\end{tabular}
\end{table}
...
$1.781$ and $1.798$\,mm for state- and time-driven execution, respectively.
Under holdback it is $2.831$ and $3.248$\,mm. The corresponding full Cartesian
RMSE increases to $26.533$ and $28.669$\,mm. Thus this 120\,N test demonstrates
...
cut-depth RMSE is $2.369$\,mm for state-driven timing and $2.370$\,mm for
time-driven timing. Under holdback the corresponding values are $4.733$ and
$6.530$\,mm, and full Cartesian RMSE is $84.593$ and $114.292$\,mm. Workpiece
...
zero.  The state-driven law has lower mean cut-depth RMSE in three cells but
higher error in moving nominal execution. Full Cartesian RMSE is lower in the
two perturbed cells and higher in both nominal cells. None of the paired
cut-depth comparisons is significant ($p\in[0.0625,0.8125]$). The large
seed-to-seed dispersion, including a moving
...
$\dot q^\top\tau$ exceeds the counterfactual budget for $69.8\%$ of samples
and reaches a maximum margin of $+24{,}586$\,W. With torque projection
...
at $\phi=0.024$ after storage depletion; safety intervention therefore does not
...
\midrule
No projection & $100\%$ & $0.001$ & $69.8\%$ & $+24{,}586$\\
Tank projection & $100\%$ & $0.001$ & $0.0\%$ & $0$\\
Tank + phase gate & $0\%$ ($\phi_f=0.024$) & $0.001$ & $0.0\%$ & $0$\\
\bottomrule
\end{tabular}
...
precision has the lowest mean cut-depth error: $6.0\%$, $33.8\%$ and $38.6\%$
below the permuted, inverted and direct-force comparators, respectively.
Paired significance is $p=0.001$, $0.001$ and $0.002$; absolute error remains
highly dispersed.
...
$1.781$--$2.370$\,mm in the four-case cork comparison. Full Cartesian error is
larger, and the 120\,N holdback produces $26.533$--$114.292$\,mm RMSE; no
...
violates the available energy budget for $69.8\%$ of the samples
($+24{,}586$\,W maximum margin), while both tanked controllers satisfy
$\dot q^\top\tau_{\rm safe}\le P_{\max}$ for the whole run. With the deliberately
small 1\,J stress-test tank, the phase-gated case halts at $\phi=0.024$ rather
than completing.
...
Representative cut-depth RMSE is $1.781$, $2.831$, $2.369$ and $4.733$\,mm,
