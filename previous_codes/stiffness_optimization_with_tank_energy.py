...
class optimal_stiffness:
    ...
    def set_weights(self, q_factor: int = 3200, r_factor: int = 1, coriolis_factor=0.1):
        self.Q *= q_factor
        self.R *= r_factor
...
#     # Example data and running the optimizer
#     n = 3  # Dimension of k^d
#     x_tilde = np.array([1.0, 2.0, 3.0])  # Example values for the Cartesian position error
#     F_d = np.array([15, 15, 50])  # Desired forces
#     K_min = np.array([0.5, 0.5, 0.5])  # Minimum stiffness
#     k_max = np.array([1000.0, 1000.0, 1000.0])  # Maximum stiffness
#     epsilon = 0.3  # Minimum energy threshold
#     delta_t = 0.1  # Time step

#     optimizer = optimal_stiffness(n, K_min, k_max, epsilon, delta_t, cp.OSQP)
#     k_d_opt, opt_solution, status = optimizer.run(x_tilde, F_d, use_constraints=True)
...
if __name__ == "__main__":
    ...
    x_tilde_demo = np.array([[0.1,0.1,i] for i in x_tr])
    kd_opt_arr = []
    ...
    for x_tilde, F_d in zip(x_tilde_demo, F_d_demo):
        try:
            k_d_opt, opt_solution, status, tank_energy = stiffness_optimizer.run(x_tilde, F_d, verbose=False, use_constraints=True)
